...
const _CabinetFrame3D: React.FC<CabinetFrame3DProps> = ({
  uHeight,
  width,
  depth,
}) => {
  ...
  return (
    <group>
      ...
      {[
        {
          pos: [-width / 2 - 0.01, height / 2, 0],
          size: [0.02, height, depth] as [number, number, number],
        },
        {
          pos: [width / 2 + 0.01, height / 2, 0],
          size: [0.02, height, depth] as [number, number, number],
        ...
        {
          pos: [0, height + 0.01, 0],
          size: [width + 0.04, 0.02, depth] as [number, number, number],
        },
        {
          pos: [0, -0.01, 0],
          size: [width + 0.04, 0.02, depth] as [number, number, number],
...
const _EmptySlot3D: React.FC<EmptySlot3DProps> = ({
  startU,
  endU,
  width,
  depth,
    ...
    <>
      ...
      >
        ...
        <mesh position={[0, cabinetHeight / 2, cabinetDepth / 2 + 0.01]}>
          <boxGeometry
            args={[cabinetWidth - 0.02, cabinetHeight - 0.02, 0.02]}
          />
          <meshStandardMaterial
            color="#2d3748"
            metalness={0.4}
        ...
        <mesh position={[0, cabinetHeight + 0.01, 0]}>
          <boxGeometry
            args={[cabinetWidth - 0.05, 0.02, cabinetDepth - 0.05]}
          />
          <meshStandardMaterial
            color="#1a202c"
            metalness={0.5}
        ...
        {[
          ...
          <mesh
            key={idx}
            position={[
              x * (cabinetWidth / 2 - 0.03),
              -0.02,
              z * (cabinetDepth / 2 - 0.03),
            ]}
          >
        ...
        {emptySlots.map((slot, idx) => {
          const height =
            (slot.endU - slot.startU + 1) * U_HEIGHT * DISPLAY_SCALE;
          const y =
            ((slot.startU + slot.endU) / 2 - 0.5) * U_HEIGHT * DISPLAY_SCALE;
          return (
            <mesh
              key={`empty-${idx}`}
              position={[0, y, cabinetDepth / 2 + 0.02]}
            >
        ...
        {devices.map((device) => {
          ...
          const deviceHeight =
            (device.endU - device.startU + 1) * U_HEIGHT * DISPLAY_SCALE;
          const deviceY =
            ((device.startU + device.endU) / 2 - 0.5) *
            U_HEIGHT *
            DISPLAY_SCALE;
...
const Cabinet3DPage: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      usedU,
      totalU,
      usage: Math.round((usedU / totalU) * 100),
    };
  }, [devices, cabinet]);
