...
declare namespace IDC {
  ...
  interface PortGroup {
    id: string;
    name: string;           // 如"千兆电口组"
    portType: 'RJ45' | 'SFP' | 'SFP+' | 'QSFP+' | 'QSFP28' | 'FC' | 'USB' | 'Console' | 'Power';
    count: number;          // 端口数量
  ...
  interface DeviceTemplate {
    id: string;
    name: string;           // 模板名称
    category: 'switch' | 'router' | 'server' | 'storage' | 'firewall' | 'loadbalancer' | 'other';
    brand: string;          // 品牌
    model: string;          // 型号
    uHeight: number;        // U位高度
  ...
  /** 连线类型 */
  type ConnectionType = 'network' | 'power' | 'management' | 'storage' | 'stack';

  /** 线缆类型 */
  type CableType = 'Cat5e' | 'Cat6' | 'Cat6a' | 'Cat7' | 'SingleModeFiber' | 'MultiModeFiber' | 'DAC' | 'AOC' | 'PowerCable';
