...
import { Button, message, Popconfirm, Tag, Space, Card, Descriptions, Modal, List, Badge } from 'antd';
...
const DeviceTemplatePage: React.FC = () => {
    ...
    const addPortGroup = () => {
        setPortGroups([...portGroups, { name: '', portType: 'RJ45', count: 1, speed: '1G' }]);
    };
        ...
        >
            <ProTable<IDC.DeviceTemplate>
                ...
                toolBarRender={() => [
                    <Button
                        ...
                        icon={<Plus size={16} />}
                        onClick={() => {
                            setPortGroups([{ name: '', portType: 'RJ45', count: 1, speed: '1G' }]);
            ...
            >
                ...
                <Card
                    ...
                    extra={
                        <Button type="link" onClick={addPortGroup} icon={<Plus size={14} />}>
                            添加端口组
                        </Button>
                    }
                >
                    {portGroups.map((pg, index) => (
                        <div key={`${pg.portType}-${pg.name}-${index}`} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                            <input
                                placeholder="端口组名称"
                                value={pg.name}
                                onChange={e => updatePortGroup(index, 'name', e.target.value)}
                                style={{ width: 120, padding: '4px 8px', border: '1px solid #d9d9d9', borderRadius: 4 }}
                            />
                            <select
                                value={pg.portType}
                                onChange={e => updatePortGroup(index, 'portType', e.target.value)}
                                style={{ width: 130, padding: '4px 8px', border: '1px solid #d9d9d9', borderRadius: 4 }}
                            >
                            ...
                            <input
                                ...
                                value={pg.count}
                                min={1}
                                onChange={e => updatePortGroup(index, 'count', parseInt(e.target.value, 10) || 1)}
                                style={{ width: 60, padding: '4px 8px', border: '1px solid #d9d9d9', borderRadius: 4 }}
                            />
                            <select
                                value={pg.speed}
                                onChange={e => updatePortGroup(index, 'speed', e.target.value)}
                ...
                {currentRow && (
                    <>
                        <Descriptions bordered column={2} size="small">
                            ...
                            <Descriptions.Item label="品牌">{currentRow.brand}</Descriptions.Item>
                            <Descriptions.Item label="型号">{currentRow.model}</Descriptions.Item>
                            <Descriptions.Item label="U位高度">{currentRow.uHeight}U</Descriptions.Item>
                        ...
                        <Card title="端口配置" size="small" style={{ marginTop: 16 }}>
                            <List
                                ...
                                renderItem={(pg) => (
                                    <List.Item>
                                        <List.Item.Meta
                                            avatar={<Badge count={pg.count} style={{ backgroundColor: '#722ed1' }} />}
