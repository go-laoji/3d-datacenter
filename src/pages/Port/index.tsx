...
import { Card, Select, Table, Tag, Space, Button, Modal, Form, Input, InputNumber, message, Badge, Tooltip } from 'antd';
...
import { getPortsByDevice, updatePort, batchUpdatePortVlan } from '@/services/idc/port';
...
const PortPage: React.FC = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (status: string) => (
                <Badge status={statusColors[status] as any} text={
                    status === 'up' ? '启用' : status === 'down' ? '关闭' : status === 'disabled' ? '禁用' : '错误'
                } />
            ),
        },
        {
        ...
        },
        {
            ...
            render: (mode: string) => mode ? (
                <Tag color={mode === 'access' ? 'blue' : mode === 'trunk' ? 'purple' : 'cyan'}>
                    {mode.toUpperCase()}
                </Tag>
            ) : '-',
        },
        ...
        },
        {
            ...
            width: 150,
            ellipsis: true,
            render: (vlans: number[]) => vlans?.length ? (
                <Tooltip title={vlans.join(', ')}>
                    {vlans.slice(0, 3).join(', ')}{vlans.length > 3 ? `...+${vlans.length - 3}` : ''}
                </Tooltip>
            ) : '-',
        },
        {
                ...
                <Button
                    ...
                    onClick={() => {
                        ...
                        form.setFieldsValue({
                            ...
                            allowedVlans: record.vlanConfig?.allowedVlans?.join(', '),
    ...
    const handleSaveConfig = async () => {
        ...
        const allowedVlans = values.allowedVlans
            ? values.allowedVlans.split(',').map((v: string) => parseInt(v.trim(), 10)).filter((v: number) => !Number.isNaN(v))
            : undefined;
        ...
        >
            ...
            >
                <Form form={form} layout="vertical">
                    ...
                    <Form.Item name="allowedVlans" label="允许VLAN" tooltip="多个VLAN用逗号分隔，如: 1, 100, 200">
                        <Input placeholder="如: 1, 100, 200, 300" />
                    </Form.Item>
