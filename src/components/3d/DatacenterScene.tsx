...
// 设备悬停Tooltip组件
const DeviceTooltip: React.FC<{ device: IDC.Device; visible: boolean }> = ({ device, visible }) => {
...
interface CabinetProps {
    ...
    onDoubleClick: (cabinet: IDC.Cabinet, position: [number, number, number]) => void;
    onDeviceSelect: (device: IDC.Device) => void;
    onDeviceDoubleClick: (device: IDC.Device, position: [number, number, number]) => void;
}
...
export const Cabinet3D: React.FC<CabinetProps> = ({
    ...
    const usageColor = usageRate > 0.9 ? '#f5222d' : usageRate > 0.7 ? '#faad14' : '#52c41a';
    ...
    return (
        <group ref={groupRef} position={position}>
            ...
            {(selected || highlighted) && (
                <lineSegments>
                    <edgesGeometry args={[new THREE.BoxGeometry(cabinetWidth + 0.02, cabinetHeight + 0.02, cabinetDepth + 0.02)]} />
                    <lineBasicMaterial color={selected ? '#4096ff' : '#52c41a'} linewidth={2} />
                </lineSegments>
            )}
            ...
            <mesh position={[-cabinetWidth / 2 - 0.02, 0, cabinetDepth / 2]}>
                ...
                <meshStandardMaterial color={usageColor} emissive={usageColor} emissiveIntensity={0.3} />
            </mesh>

            {/* 状态指示灯 */}
            <mesh position={[cabinetWidth / 2 - 0.05, cabinetHeight / 2 + 0.02, cabinetDepth / 2]}>
                <sphereGeometry args={[0.02, 16, 16]} />
                <meshStandardMaterial
                    color={statusColors[cabinet.status]}
                    emissive={statusColors[cabinet.status]}
                    emissiveIntensity={isWarning ? 1.5 : 0.8}
                />
            </mesh>

            {/* 使用新的3D设备模型渲染 */}
            {devices.map((device) => {
                const deviceY = ((device.startU - 1) * 0.0445) - (cabinetHeight / 2) + ((device.endU - device.startU + 1) * 0.0445 / 2);
                ...
                return (
                    <Device3D
                        ...
                        onDoubleClick={(d, _pos) => {
                            const devicePos: [number, number, number] = [position[0], position[1] + deviceY, position[2]];
            ...
            >
                <div style={{
                    background: selected ? '#1890ff' : highlighted ? '#52c41a' : 'rgba(0,0,0,0.7)',
                    color: '#fff',
                    padding: '2px 8px',
                    borderRadius: 4,
                    fontSize: 12,
...
// 数据中心3D场景
export const DatacenterScene = forwardRef<DatacenterSceneRef, DatacenterSceneProps>(({
    cabinets,
    devices,
    connections,
    templates,
    ...
    // 双击聚焦
    const handleCabinetDoubleClick = useCallback((_cabinet: IDC.Cabinet, position: [number, number, number]) => {
        setCameraTarget(position);
    }, []);

    const handleDeviceDoubleClick = useCallback((_device: IDC.Device, position: [number, number, number]) => {
        setCameraTarget(position);
    }, []);
    ...
    return (
        <>
            ...
            {/* 地板 */}
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[4, -0.01, 4]} receiveShadow>
                <planeGeometry args={[20, 20]} />
                <meshStandardMaterial color="#d0d8e0" metalness={0.1} roughness={0.9} />
            </mesh>
            ...
            {cabinets.map((cabinet) => (
                <Cabinet3D
                    ...
                    highlighted={highlightedCabinetId === cabinet.id ||
                        getDevicesByCabinet(cabinet.id).some(d => d.id === highlightedDeviceId)}
            ...
            {connections.map((conn) => {
                ...
                return (
                    <mesh key={conn.id}>
                        <tubeGeometry args={[curve, 20, isHighlighted ? 0.015 : 0.01, 8, false]} />
                        <meshStandardMaterial
