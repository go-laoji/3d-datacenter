...
import { ProTable, ModalForm, ProFormText, ProFormTextArea, ProFormSelect, ProFormDatePicker, } from '@ant-design/pro-components';
import type { ProColumns, ActionType } from '@ant-design/pro-components';
import { Button, message, Popconfirm, Tag, Space, Tooltip, Drawer, Descriptions, Badge, Alert, } from 'antd';
...
const statusConfig: Record<string, { color: string; text: string; icon: React.ReactNode }> = {
    ...
    warning: { color: 'warning', text: '告警', icon: <AlertTriangle size={14} /> },
    error: { color: 'error', text: '故障', icon: <AlertTriangle size={14} /> },
    maintenance: { color: 'processing', text: '维护中', icon: <Settings size={14} /> },
};
...
const USlotSelector: React.FC<{
    ...
    const [uUsage, setUUsage] = useState<{ u: number; occupied: boolean; deviceName?: string }[]>([]);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (cabinetId) {
            setLoading(true);
            getCabinetUUsage(cabinetId).then(res => {
                if (res.success && res.data) {
                    // API返回的是对象 { uSlots: [...] }，需要提取并转换
                    const slots = res.data.uSlots || [];
                    setUUsage(slots.map((s: any) => ({
                        u: s.u,
                        occupied: !!s.deviceId,
    ...
    if (!cabinetId) return <Alert message="请先选择目标机柜" type="info" showIcon />;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', maxHeight: 200, overflowY: 'auto' }}>
                {Array.from({ length: uHeight }, (_, i) => uHeight - i).map(u => {
                    ...
                    const isSelected = selectedStartU && u >= selectedStartU && u < selectedStartU + deviceUHeight;
                    const canStart = !isOccupied && isSlotAvailable(u);

                    return (
                        <Tooltip
                            key={u}
                            title={isOccupied ? `已被 ${slot.deviceName} 占用` : canStart ? `点击选择 U${u} 作为起始位置` : '不可用'}
                        >
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundColor: isSelected ? '#1890ff' : isOccupied ? '#ffccc7' : '#f0f0f0',
...
const DevicePage: React.FC = () => {
    ...
    const columns: ProColumns<IDC.Device>[] = [
        ...
        },
        {
            ...
            search: false,
            render: (_, record) =>
                record.startU === record.endU ? `U${record.startU}` : `U${record.startU}-${record.endU}`,
        },
        {
            title: '管理IP',
            ...
            width: 130,
            copyable: true,
            render: (_, record) => record.managementIp && (
                <Tooltip title="点击复制">
                    <Space>
            ...
            render: (_, record) => {
                ...
                return (
                    <Tooltip title={isExpired ? '已过期' : `剩余${daysLeft}天`}>
                        <span style={{ color: isExpired ? '#f5222d' : isExpiringSoon ? '#faad14' : undefined }}>
        ...
        >
            ...
            >
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <div>
                        ...
                        {selectedCabinetId && selectedTemplateId && (
                            <div style={{ marginBottom: 24 }}>
                                <div style={{ marginBottom: 8, fontWeight: 500 }}>选择起始U位 <span style={{ color: '#f5222d' }}>*</span></div>
                                <USlotSelector
                    ...
                    <div>
                        <ProFormText name="serialNumber" label="序列号" placeholder="请输入设备序列号" />
                        <ProFormText name="managementIp" label="管理IP" placeholder="如：10.0.1.1" />
            ...
            >
                ...
                <ProFormText name="assetCode" label="资产编码" rules={[{ required: true }]} />
                ...
                {currentRow && (
                    <Descriptions bordered column={1} size="small">
                        <Descriptions.Item label="设备名称">
                            <Space>
                                <Badge status={currentRow.status === 'online' ? 'success' : 'default'} />
                                {currentRow.name}
                            </Space>
                        </Descriptions.Item>
                        <Descriptions.Item label="资产编码">{currentRow.assetCode}</Descriptions.Item>
                        <Descriptions.Item label="序列号">{currentRow.serialNumber || '-'}</Descriptions.Item>
                        <Descriptions.Item label="设备型号">
                            {templates.find(t => t.id === currentRow.templateId)?.name || currentRow.templateId}
                        </Descriptions.Item>
                        <Descriptions.Item label="所在机柜">
                            {cabinets.find(c => c.id === currentRow.cabinetId)?.name || currentRow.cabinetId}
                        </Descriptions.Item>
                        <Descriptions.Item label="U位">
                            U{currentRow.startU} - U{currentRow.endU}
                        </Descriptions.Item>
                        <Descriptions.Item label="管理IP">{currentRow.managementIp || '-'}</Descriptions.Item>
                        <Descriptions.Item label="状态">
                            <Tag color={statusConfig[currentRow.status]?.color}>
                                {statusConfig[currentRow.status]?.text}
                            </Tag>
                        </Descriptions.Item>
                        <Descriptions.Item label="采购日期">{currentRow.purchaseDate || '-'}</Descriptions.Item>
                        <Descriptions.Item label="质保到期">{currentRow.warrantyExpiry || '-'}</Descriptions.Item>
                        <Descriptions.Item label="供应商">{currentRow.vendor || '-'}</Descriptions.Item>
                        ...
                        <Descriptions.Item label="备注">{currentRow.description || '-'}</Descriptions.Item>
                    </Descriptions>
                )}
            </Drawer>
            ...
            <DevicePortView
                ...
                template={portViewDevice ? templates.find(t => t.id === portViewDevice.templateId) : null}
                open={portViewOpen}
                onClose={() => {
