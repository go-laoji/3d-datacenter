...
import { Card, Select, Space, Drawer, Descriptions, Tag, Badge, Button, Spin, Empty, Tooltip, message, Progress, Divider } from 'antd';
...
import { DatacenterScene, type DatacenterSceneRef } from '@/components/3d/DatacenterScene';
...
// 端口面板组件
const PortPanel: React.FC<{ device: IDC.Device; template: any }> = ({ device, template }) => {
...
const Datacenter3DPage: React.FC = () => {
    ...
    const [selectedDc, setSelectedDc] = useState<string | undefined>(dcId || undefined);
    ...
    const [selectedCabinet, setSelectedCabinet] = useState<IDC.Cabinet | null>(null);
        ...
        if (selectedDc) {
            ...
            Promise.all([
                ...
                if (devRes.success) {
                    ...
                    setDevices((devRes.data || []).filter((d: any) => cabinetIds.includes(d.cabinetId)));
                }
            ...
            <Card className={styles.card3d}>
                ...
                <div className={styles.toolbar}>
                            ...
                            options={searchResults.map(d => ({
                                ...
                                label: (
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <span>{d.name}</span>
                                        <span style={{ color: '#8c8c8c', fontSize: 12 }}>{d.managementIp || d.assetCode}</span>
                                    </div>
                                ),
                            }))}
                        />
                    ...
                    <Space>
                        <Tooltip title={showConnections ? '隐藏连线' : '显示连线'}>
                            <Button
                                icon={showConnections ? <Eye size={16} /> : <EyeOff size={16} />}
                        ...
                        <Tooltip title="重置视角">
                            <Button icon={<RotateCcw size={16} />} onClick={handleResetCamera}>
                                重置
                            </Button>
                        </Tooltip>
                    </Space>
                ...
                <div className={styles.stats}>
                    ...
                    <div className={styles.statItem}>
                        ...
                        <span>告警: {devices.filter(d => d.status === 'warning').length}</span>
                    </div>
                    <div className={styles.statItem}>
                        <Badge status="error" />
                        <span>故障: {devices.filter(d => d.status === 'error').length}</span>
                    </div>
                </div>
                ...
                <div className={styles.hint}>
                    <span>💡 操作提示：单击选择 | 双击聚焦 | 右键拖动旋转 | 滚轮缩放 | 悬停查看详情</span>
                </div>
            </Card>
                ...
                {selectedCabinet && (
                    <>
                        <Descriptions bordered column={1} size="small">
                            <Descriptions.Item label="机柜名称">{selectedCabinet.name}</Descriptions.Item>
                            <Descriptions.Item label="机柜编码">{selectedCabinet.code}</Descriptions.Item>
                            <Descriptions.Item label="位置">{selectedCabinet.row}排{selectedCabinet.column}列</Descriptions.Item>
                            <Descriptions.Item label="U位使用">
                                <div style={{ width: '100%' }}>
                                    <Progress
                                        percent={Math.round((selectedCabinet.usedU / selectedCabinet.uHeight) * 100)}
                                        size="small"
                                        status={selectedCabinet.usedU / selectedCabinet.uHeight > 0.9 ? 'exception' : 'active'}
                                    />
                            ...
                            <Descriptions.Item label="功率使用">
                                <div style={{ width: '100%' }}>
                                    <Progress
                                        percent={Math.round((selectedCabinet.currentPower / selectedCabinet.maxPower) * 100)}
                                        size="small"
                                        strokeColor="#faad14"
                                    />
                                    <span style={{ fontSize: 12, color: '#8c8c8c' }}>
                                        {selectedCabinet.currentPower}W / {selectedCabinet.maxPower}W
                                    </span>
                                </div>
                            </Descriptions.Item>
                        ...
                        {/* U位图 */}
                        <Card
                            title={<Space><LayoutGrid size={14} />U位占用图</Space>}
                            size="small"
                            style={{ marginTop: 16 }}
                        >
                            <USlotDiagram
                    ...
                    <>
                        <Descriptions bordered column={1} size="small">
                            <Descriptions.Item label="设备名称">
                                <Space>
                                    <Badge status={selectedDevice.status === 'online' ? 'success' : 'default'} />
                                    {selectedDevice.name}
                                </Space>
                            </Descriptions.Item>
                            <Descriptions.Item label="资产编码">{selectedDevice.assetCode}</Descriptions.Item>
                            <Descriptions.Item label="设备型号">{getTemplateName(selectedDevice.templateId)}</Descriptions.Item>
                            <Descriptions.Item label="所在机柜">{getCabinetName(selectedDevice.cabinetId)}</Descriptions.Item>
                            <Descriptions.Item label="U位">U{selectedDevice.startU} - U{selectedDevice.endU}</Descriptions.Item>
                            <Descriptions.Item label="管理IP">{selectedDevice.managementIp || '-'}</Descriptions.Item>
                            <Descriptions.Item label="状态">
                        ...
                        {/* 端口面板 */}
                        <Card
                            title={<Space><MonitorUp size={14} />端口面板</Space>}
                            size="small"
                            style={{ marginTop: 16 }}
                        >
                            <PortPanel
                        ...
                        {/* 设备连线 */}
                        <Card title={<Space><Cable size={14} />设备连线</Space>} size="small" style={{ marginTop: 16 }}>
                            {connections.filter(c =>
                                c.sourceDeviceId === selectedDevice.id || c.targetDeviceId === selectedDevice.id
                            ).map(conn => {
                                const otherDeviceId = conn.sourceDeviceId === selectedDevice.id
                                    ? conn.targetDeviceId
                                    : conn.sourceDeviceId;
                            ...
                            {connections.filter(c =>
                                c.sourceDeviceId === selectedDevice.id || c.targetDeviceId === selectedDevice.id
                                    ...
                                    <Empty description="暂无连线" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                                )}
                        </Card>
                        ...
                        <div style={{ marginTop: 16 }}>
                            <Space>
                                <Button
                                    ...
                                    onClick={() => history.push(`/network/port?device=${selectedDevice.id}`)}
                                >
                                    配置端口
                                </Button>
                                <Button
                                    ...
                                    onClick={() => {
                                        const cabinet = cabinets.find(c => c.id === selectedDevice.cabinetId);
                                        if (cabinet && sceneRef.current) {
                                            const rowSpacing = 1.5;
                                            const colSpacing = 0.8;
                                            const x = (cabinet.column - 1) * colSpacing;
