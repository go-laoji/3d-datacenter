...
const ResourceTree: React.FC = () => {
        ...
        const nodes: DataNode[] = res.data.map((dc: any) => ({
          title: (
            <span className={styles.nodeTitle}>
              ...
              <Tag color="blue" className={styles.nodeTag}>
                {dc.code}
              </Tag>
            </span>
          ),
          ...
          const children: DataNode[] = res.data.map((cab: any) => ({
            title: (
              <span className={styles.nodeTitle}>
                ...
                <Tag
                  color={statusColors[cab.status] || 'default'}
      ...
      try {
        ...
        if (res.success && res.data) {
          const children: DataNode[] = res.data.map((dev: any) => {
            ...
            const statusConf =
              deviceStatusConfig[dev.status] || deviceStatusConfig.offline;
            return {
              title: (
                <span className={styles.nodeTitle}>
                  <span
                  ...
                  <Tag
                    color={statusColors[dev.status] || 'default'}
                    className={styles.nodeTag}
                  >
                    {dev.status}
                  </Tag>
  ...
  // 递归收集所有节点的 key 和名称
  const collectNodeInfo = (
    nodes: DataNode[],
  ): { key: React.Key; name: string; parentKeys: React.Key[] }[] => {
    const result: { key: React.Key; name: string; parentKeys: React.Key[] }[] =
      [];
    const traverse = (list: DataNode[], parents: React.Key[]) => {
      for (const node of list) {
        const data = (node as any).data;
        const name = data?.name || data?.code || '';
        result.push({ key: node.key, name, parentKeys: [...parents] });
        if (node.children) {
            ...
            suffix={
              searchValue ? (
                <Badge
                  ...
                  style={{
                    backgroundColor: searchMatchCount > 0 ? '#52c41a' : '#999',
                  }}
                />
              ) : null
            }
