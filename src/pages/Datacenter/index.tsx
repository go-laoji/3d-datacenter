...
const DatacenterPage: React.FC = () => {
  ...
  const columns: ProColumns<IDC.Datacenter>[] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const percent =
          record.totalCabinets > 0
            ? Math.round((record.usedCabinets / record.totalCabinets) * 100)
            : 0;
        return (
          <Tooltip
            title={`已用 ${record.usedCabinets} / 共 ${record.totalCabinets}`}
          >
            <Progress
              percent={percent}
              size="small"
              ...
              format={() => `${record.usedCabinets}/${record.totalCabinets}`}
            />
          </Tooltip>
        );
      },
      ...
      <ModalForm
        ...
        onFinish={async (values) => {
          const res = await createDatacenter(
            values as IDC.DatacenterCreateParams,
          );
          if (res.success) {
