...
export const CabinetFrontViewContent: React.FC<
  CabinetFrontViewContentProps
> = ({ cabinet, devices, templates, onDeviceClick, compact = false }) => {
        ...
        >
          ...
          >
            ...
            <div className={styles.uLabel}>
              {device.startU === device.endU
                ? `U${device.startU}`
                : `U${device.startU}-${device.endU}`}
            </div>
          </div>
        </Tooltip>
      );
  ...
  const powerPercent = Math.round(
    (cabinet.currentPower / cabinet.maxPower) * 100,
  );

  const handleJumpToU = (u: number) => {
    const offset = Math.max(0, (cabinet.uHeight - u) * U_HEIGHT - 24);
    ...
    <div className={`${styles.container} ${compact ? styles.compact : ''}`}>
      ...
      <div className={styles.header}>
        <div className={styles.cabinetInfo}>
          ...
          <div className={styles.cabinetMeta}>
            ...
            <span>
              位置: {cabinet.row}排{cabinet.column}列
            </span>
      ...
      {!compact && (
        <div className={styles.viewToolbar}>
          <Segmented
            value={viewSide}
            onChange={(value) => setViewSide(value as 'front' | 'rear')}
            options={[
              { label: '正面视图', value: 'front' },
              { label: '背面视图', value: 'rear' },
            ]}
          />
          <Space>
            <span className={styles.jumpLabel}>快速跳转</span>
            <Select
              aria-label="快速跳转到 U 位"
              placeholder="选择 U 位"
              style={{ width: 112 }}
              onChange={handleJumpToU}
              options={Array.from({ length: cabinet.uHeight }, (_, index) => ({
                value: cabinet.uHeight - index,
                label: `U${cabinet.uHeight - index}`,
              }))}
            />
          </Space>
        </div>
      ...
      <div
        ref={viewportRef}
        className={`${styles.cabinetViewport} ${compact ? styles.compactViewport : ''}`}
      >
        <div
          className={`${styles.cabinetFront} ${viewSide === 'rear' ? styles.rearView : ''}`}
        >
          {/* 左侧U位标尺 */}
          <div className={styles.uRuler}>
            {Array.from(
              { length: cabinet.uHeight },
              (_, i) => cabinet.uHeight - i,
            ).map((u) => (
          ...
          <div className={styles.cabinetBody}>
            <div className={styles.slotsLayer}>{emptySlots}</div>
            <div className={styles.devicesLayer}>{deviceElements}</div>
          </div>
