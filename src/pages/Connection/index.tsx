...
import { ArrowRight, Cable, Edit3, Monitor, Plus, Trash2 } from 'lucide-react';
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ marginBottom: 8, fontWeight: 500 }}>
        {label}端口 <span style={{ color: '#f5222d' }}>*</span>
      </div>
      {loading ? (
        <div>加载端口列表...</div>
      ) : (
...
const ConnectionPage: React.FC = () => {
  ...
  const columns: ProColumns<IDC.Connection>[] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const ct = connectionTypes.find(
          (c) => c.value === record.connectionType,
        );
        return (
          <Tag color={ct?.color}>{ct?.label || record.connectionType}</Tag>
        );
      },
    },
    {
    ...
    },
    {
      ...
      search: false,
      render: (_, record) =>
        record.cableLength ? `${record.cableLength}m` : '-',
    },
    {
      title: '颜色',
        ...
        >
          ...
          <div>
            ...
            <ProFormSelect
              name="connectionType"
              label="连线类型"
              options={connectionTypes.map((ct) => ({
                value: ct.value,
                label: ct.label,
              }))}
