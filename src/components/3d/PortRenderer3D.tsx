...
// 端口类型配置
const portTypeConfigs: Record<string, {
    width: number;
    height: number;
    depth: number;
    color: string;
    ...
    RJ45: { width: 0.012, height: 0.010, depth: 0.004, color: '#3a3f44' },
    SFP: { width: 0.008, height: 0.012, depth: 0.004, color: '#2a2f35' },
    'SFP+': { width: 0.008, height: 0.012, depth: 0.004, color: '#2a2f35', borderColor: '#1890ff' },
    'QSFP+': { width: 0.018, height: 0.012, depth: 0.004, color: '#2a2f35', borderColor: '#52c41a' },
    QSFP28: { width: 0.018, height: 0.012, depth: 0.004, color: '#2a2f35', borderColor: '#722ed1' },
    FC: { width: 0.010, height: 0.010, depth: 0.004, color: '#2a2f35', borderColor: '#d4af37' },
    USB: { width: 0.008, height: 0.006, depth: 0.003, color: '#1a1a1a' },
    Console: { width: 0.012, height: 0.010, depth: 0.004, color: '#4a90d9' },
    Power: { width: 0.020, height: 0.015, depth: 0.005, color: '#1a1a1a' },
};
...
const getPortStatus = (port: IDC.Port) => {
    ...
    if (port.status === 'up' && port.linkStatus === 'connected') return portStatusColors.up_connected;
...
export const Port3D: React.FC<Port3DProps> = ({ port, position, scale = 1 }) => {
    ...
    return (
        <group position={position}>
            ...
            {config.borderColor && (
                <lineSegments>
                    <edgesGeometry args={[new THREE.BoxGeometry(width + 0.001, height + 0.001, depth)]} />
...
export const PortGroupRenderer: React.FC<PortGroupRendererProps> = ({
    ...
    const spacing = (direction === 'horizontal' ? config.width : config.height) * 1.3 * scale;
    ...
    return (
        <group position={startPosition}>
            {groupPorts.map((port, index) => {
                ...
                let x = col * spacing;
                let y = -row * (config.height * 1.3 * scale);
                let z = 0;
...
export const FrontPortPanel: React.FC<DevicePortPanelProps> = ({
    ...
    const layout = useMemo(() => {
        // 过滤出前面板端口（非电源/管理端口）
        const frontGroups = portGroups.filter(g =>
        ...
        const result: { group: IDC.PortGroup; position: [number, number, number] }[] = [];
...
export const RearPortPanel: React.FC<DevicePortPanelProps> = ({
    ...
    const layout = useMemo(() => {
        ...
        if (rearGroups.length === 0) {
            return [{
                type: 'power_default',
                position: [width / 4, 0, -depth / 2 - 0.002] as [number, number, number],
            }];
        }
    ...
    return (
        <group>
            ...
            {Array.from({ length: Math.floor(width / 0.03) }, (_, i) => (
                <mesh
                    key={`vent-${i}`}
                    position={[-width / 2 + 0.02 + i * 0.025, -height / 4, -depth / 2 - 0.002]}
                >
            ...
            <group position={[width / 3, 0, -depth / 2 - 0.003]}>
                ...
                <mesh position={[0.025, height * 0.25, 0.006]}>
                    ...
                    <meshStandardMaterial color="#52c41a" emissive="#52c41a" emissiveIntensity={0.6} />
                </mesh>
            </group>
            ...
            <group position={[-width / 3, height * 0.2, -depth / 2 - 0.003]}>
                {/* Console口 */}
                <mesh position={[0, 0, 0]}>
                    <boxGeometry args={[0.012, 0.010, 0.004]} />
                    <meshStandardMaterial color="#4a90d9" metalness={0.3} roughness={0.5} />
                </mesh>
                {/* 管理网口 */}
                <mesh position={[0.02, 0, 0]}>
                    <boxGeometry args={[0.012, 0.010, 0.004]} />
                    <meshStandardMaterial color="#3a3f44" metalness={0.4} roughness={0.6} />
                </mesh>
            </group>

            {/* 端口组渲染 */}
            {layout.filter(item => 'group' in item).map(({ group, position }) => (
                <PortGroupRenderer
                    key={group!.id}
                    ports={ports}
                    portGroup={group!}
